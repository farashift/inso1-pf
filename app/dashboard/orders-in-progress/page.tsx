  ...
  return (
    <ProtectedLayout>
      <div className="p-6 md:p-8">
        <h1 className="text-3xl font-bold text-[#3d3330] mb-6">PEDIDOS EN PROCESO</h1>
            ...
            {orders.map((order) => (
              <div key={order.id} className="bg-white rounded-lg p-6 shadow">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <p className="text-lg font-bold text-[#3d3330]">#{order.orderNumber}</p>
                ...
                <div className="mb-4 pb-4 border-b-2 border-[#d4a574]">
                  <p className="text-sm font-bold text-[#3d3330] mb-2">Productos:</p>
                  <ul className="text-sm text-[#8b6f47] space-y-1">
                    {order.items.map((item, idx) => (
                      <li key={idx}>
                        - {item.productName} (x{item.quantity}) - S/.{(item.price * item.quantity).toFixed(2)}
                      </li>
                    ))}
                  </ul>
                </div>

                <p className="font-bold text-[#3d3330] mb-4">Total: S/.{order.totalPrice.toFixed(2)}</p>
